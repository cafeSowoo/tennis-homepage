# Kakao Schedule Automation - Visible Row Multicourt Baseline

Date: 2026-05-27  
Automation id: `automation-4`  
Automation name: `카카오톡 테니스 일정 업데이트 - visible-row`  
Status: stable baseline before speed optimizations  
...
- Multi-court labels such as `AB`, `A.B`, `A/B`, and `A+B` are not unresolved;
  they are stored with the parent `court_id` and `court_unit_id=null`.
