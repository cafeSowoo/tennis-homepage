# Kakao Schedule Automation - Visible Row Start Position Baseline

Date: 2026-06-08  
Automation id: `automation-4`  
Automation name: `카카오톡 테니스 일정 업데이트 - visible-row`  
Status: completed baseline after successful full update  
...
- Multi-court labels such as `AB`, `A.B`, `A/B`, and `A+B` are stored with the
  parent `court_id` and `court_unit_id=null`.
- The schedule tab's remembered scroll position is not trusted; processing
  starts from today's date or the first future schedule.
- Strikethrough, dimmed, or not-attending display is not used as a skip reason,
  because it can mean this account declined attendance rather than schedule
  cancellation.
...
2. 그 row의 보이는 텍스트를 `row snapshot`으로 기록한다.
   ...
   - 장소/코트 텍스트
   - 일정명 일부
3. 바로 그 visible row를 Computer Use의 현재 `element_index` 더블클릭으로 연다.
...
일정 row를 열 때는 `get_app_state(app="KakaoTalk")` 직후 현재 화면에 보이는 같은 row의 `element_index`를 `click_count=2`로 클릭하고, 저장된 row index·접근성 row 순번·과거 좌표·AppleScript 클릭은 사용하지 않는다.
